...
def ina226_init(addr):
    ...
    enable the Conversion-Ready ALERT (CNVR bit in Mask/Enable). With
    CNVR set, the chip pulls its open-drain ALERT pin low at the end of
    every conversion; the wired-OR of all 4 ALERT pins is GP27, so the
    main loop can wait on a single hardware edge instead of polling.
    Reading register 06h clears CVRF and releases that chip's ALERT."""
    config = _build_ina226_config(_ina226_avg)
    ina226_write_reg(addr, INA226_REG_CONFIG, config)
    # Bit 10 (CNVR) = 1 reroutes ALERT to fire on conversion-ready. All
    # other Mask bits stay 0 — we don't use the over/under-current alert
    # function. Polarity bit (LEN/POL/APOL) defaults are correct.
    ina226_write_reg(addr, INA226_REG_MASK_EN, INA226_MASK_CNVR)
    # Reading 06h once now clears any stale CVRF latched during config
    # writes so the first post-init ALERT edge corresponds to a real new
    # conversion, not power-on residue.
    try:
...
def _arm_cnvr():
    ...
    least one INA226 talking. Idempotent."""
...
def emit_stream_line():
    ...
    fired (or the watchdog timed out). After the data read, clear CVRF
    on every chip so ALERT releases and the next falling edge will mark
    the next set of fresh conversions."""
    global _bus_counter, _cnvr_ready
    if _bus_every <= 0:
        read_bus = False
    else:
        _bus_counter += 1
