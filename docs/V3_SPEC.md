...
## 7. Current/voltage sensing — 2× ADS131M04 + 2× INA180A1
...
**High-side current-sense amplifiers:** The ADS131M04 analog inputs have
an absolute maximum of AVDD + 0.3 V = 3.6 V. The high-side shunt nodes
(`+HV`, `+HV_P1`, `+HV_P2`) run at 2.65–10 V — directly connecting them
would destroy the ADC. Two **INA180A1** current-sense amplifiers (TI,
SOT-23-5, gain = 20 V/V, CM range −0.2 V to +26 V) convert each
high-side shunt voltage to a ground-referenced signal the ADS131M04 can
read. Low-side shunts (N1, N2) have common-mode near GND and connect
directly.

**External clock:** The ADS131M04 requires an external clock on its
CLKIN pin (no internal oscillator for normal ADC operation). Both devices
share a single 8.192 MHz LVCMOS clock generated by the RP2350 PIO on
GP27 (`ADS_CLKIN`).
...
### INA180A1 signal chain (high-side shunts P1, P2)

```
+HV ──[R_SH_P1]── +HV_P1
  │                  │
  IN+    INA180    IN-       Gain = 20 V/V
           │
          OUT ──→ ADS131M04 U_ADC1 AIN0P (gain=1, single-ended vs AGND)
```

Pinout A (TI SBOS741H): 1=OUT, 2=GND, 3=IN+, 4=IN-, 5=VS

| Ref | Drives | Pin 1 OUT | Pin 2 GND | Pin 3 IN+ | Pin 4 IN- | Pin 5 VS |
|---|---|---|---|---|---|---|
| U_CSA_P1 | R_SH_P1 | `CSA_P1_OUT` | `GND` | `+HV` | `+HV_P1` | `+3V3` |
| U_CSA_P2 | R_SH_P2 | `CSA_P2_OUT` | `GND` | `+HV` | `+HV_P2` | `+3V3` |

At 3 A: V_shunt = 30 mV → V_out = 600 mV (within ADS131M04 ±1.2 V range
at gain=1). Current LSB = 143 nV / 20 / 0.01 Ω = 0.715 µA/bit.

Decoupling: 100 nF 0603 from VS to GND per device (C_CSA_P1, C_CSA_P2).
...
|---|---|---|---|---|---|---|
| CH0 | P1 shunt current | Single-ended | 1 | ±1.2 V | `CSA_P1_OUT` (INA180 output) | `AGND` |
| CH1 | P2 shunt current | Single-ended | 1 | ±1.2 V | `CSA_P2_OUT` (INA180 output) | `AGND` |
...
**High-side (P1, P2) via INA180A1 (gain=20) + ADC gain=1:**
- ADC LSB at gain=1: 1.2 V / 2²³ = **143 nV**
- Referred to shunt: 143 nV / 20 = 7.15 nV
- Current LSB = 7.15 nV / 0.01 Ω = **0.715 µA/bit**
- At 3 A: INA output = 600 mV, ADC code ≈ 4,194,304 → 22 effective bits
- INA180 offset: ±150 µV max → 150 µV / 20 = 7.5 µV → 0.75 mA (calibratable)

**Low-side (N1, N2) direct differential, ADC gain=16:**
- LSB = 75 mV / 2²³ = **8.94 nV**
...
Pin connections (both devices share SPI1 bus and CLKIN):
...
- SYNC/RESET → `ADS_RESET` (GP15, shared, active low)
- CLKIN → `ADS_CLKIN` (GP27, PIO 8.192 MHz, shared)

**U_ADC2 (bus voltages):**
- DRDY → `ADS2_DRDY` (GP21)
- CS → `ADS2_CS` (GP26)
- SCLK/DIN/DOUT/SYNC_RESET/CLKIN → shared with U_ADC1

Both devices:
- AVDD → `+3V3_A`
- DVDD → `+3V3` (external digital/IO supply)
...
| Ref | Pin 1 Vin- (GND) | Pin 2 Vin+ | Pin 3 Vout- (0V) | Pin 4 Vout+ (+Vo) | Feeds |
|---|---|---|---|---|---|
| PS1 | `GND` | `+5V` | `CELL_A_POS` | `VCC2_P1` | U1 VDD/VSS (high-side, floating) |
| PS2 | `GND` | `+5V` | `CELL_B_POS` | `VCC2_P2` | U2 VDD/VSS (high-side, floating) |
| PS3 | `GND` | `+5V` | `GND` | `VCC2_LS` | U3/U4 VDD (low-side, ground-referenced 12 V) |
...
| GP27 | `ADS_CLKIN` | PIO-generated 8.192 MHz clock → ADS131M04 CLKIN (shared) |
| GP28 | `ADC1` | Future: transient capture channel (RP2350 ADC input) |
...
| `ADS_CLKIN` | 3 (MCU GP27 + 2× ADS131M04 CLKIN) |
...
~95 parts (vs ~70 in V2). Net additions:
...
- [ ] High-side shunt common-mode (up to 10 V) exceeds ADS131M04 abs max (3.6 V) — **resolved by 2× INA180A1 CSA** (CM range to 26 V, ground-referenced output)
- [ ] INA180A1 output at 3 A: 20 × 30 mV = 600 mV — within ADS131M04 gain=1 range (±1.2 V)
- [ ] ADS131M04 requires external CLKIN — GP27 provides 8.192 MHz via PIO
